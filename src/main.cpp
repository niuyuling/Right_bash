...
void Help(char** argv) {
	static const char* help_s[] = {
		...
		"    -d      delete Registry",
		"    -b      开机启动 SSH (默认Debian)",
		"    -p      关闭开机启动 SSH (默认Debian)",
		"",
		0
	};
	static const char name[] = "Right_bash 1.1";
...
void del_boot_ssh() {
	...
	// 删除 计算机\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run 下的 ssh
...
{
	int c;
	while ((c = getopt(argc, argv, "adbph?")) != -1) {
		switch (c) {
		case 'a':
			add_Right();
			break;
